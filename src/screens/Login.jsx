    ...
    <div className="min-h-dvh flex">
      ...
      <div className="hidden lg:flex flex-1 bg-[#1A1A2E] flex-col justify-between p-14 relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 relative z-10">
          ...
          <span className="font-display text-white text-2xl font-bold">서울문화</span>
        </div>
        ...
        <div className="relative z-10">
          ...
          <div className="flex flex-wrap gap-2 mt-8">
            {['🔍 자치구·분야 검색', '❤️ 관심행사 저장', '📅 캘린더 관리', '✨ AI 소개문', '💬 댓글'].map(f => (
              <span key={f} className="text-sm font-medium text-white/60 bg-white/10 px-4 py-2 rounded-full">{f}</span>
            ))}
          </div>
        </div>
      ...
      <div className="flex-1 lg:max-w-md bg-[#1A1A2E] lg:bg-white flex flex-col relative overflow-hidden">
        ...
        <p className="text-center text-white/30 lg:text-[#9CA3AF] text-xs pb-10 px-8 leading-relaxed">
          로그인 시 <span className="underline">이용약관</span> 및 <span className="underline">개인정보 처리방침</span>에 동의하게 됩니다
        </p>
      </div>
    </div>
  )
