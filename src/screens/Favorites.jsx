...
const FAVORITE_IDS = [1, 3, 7]
const favoriteEvents = EVENTS.filter(e => FAVORITE_IDS.includes(e.id))
  ...
  return (
    <div className="bg-white rounded-2xl flex overflow-hidden shadow-sm">
      ...
      <div className="flex-1 p-3 min-w-0">
        ...
        <p className="text-[#9CA3AF] text-xs">{formatDate(event)}</p>
      </div>
      <button className="flex-shrink-0 px-3 flex items-center">
...
function EventsCalendar() {
  ...
  favoriteEvents.forEach(e => {
    const sd = parseInt(e.startDate.split('-')[2])
    const ed = parseInt(e.endDate.split('-')[2])
    const color = (CATEGORY_COLOR[e.category] ?? { text: '#FF6B47' }).text
    for (let d = sd; d <= Math.min(ed, DAYS_IN_MONTH); d++) {
      if (!dayColorMap.has(d)) dayColorMap.set(d, color)
    }
  })
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
      <div className="flex items-center justify-between px-4 pt-4 pb-2">
        <button className="w-8 h-8 flex items-center justify-center text-[#6B7280] rounded-lg">‹</button>
        <h3 className="font-display text-lg font-bold text-[#1A1A2E]">{YEAR}년 {MONTH}월</h3>
        <button className="w-8 h-8 flex items-center justify-center text-[#6B7280] rounded-lg">›</button>
      </div>
      <div className="grid grid-cols-7 px-2 mb-1">
        {DAYS_OF_WEEK.map((d, i) => (
          <div key={d} className={`text-center text-[11px] font-semibold py-1 ${i===0?'text-[#FF6B47]':i===6?'text-[#8B5CF6]':'text-[#9CA3AF]'}`}>{d}</div>
        ))}
      </div>
    ...
    <div className="flex flex-col min-h-full bg-[#FAFAF8]">
      <div className="px-5 md:px-8 lg:px-10 pt-12 pb-4 bg-[#1A1A2E]">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-white/50 text-sm mt-1">행사 {favoriteEvents.length}개 저장됨</p>
        </div>
      </div>
          ...
          <div className="max-w-5xl mx-auto px-5 md:px-8 lg:px-10 pt-4">
            {favoriteEvents.length === 0 ? (
              <div className="text-center py-16">
                ...
                <p className="text-[#6B7280] text-sm mt-3 font-medium">아직 저장한 행사가 없어요</p>
                <p className="text-[#9CA3AF] text-xs mt-1">행사 목록에서 ❤️을 눌러 저장해보세요</p>
              </div>
            ) : (
          ...
          <div className="max-w-5xl mx-auto px-5 md:px-8 lg:px-10 pt-4">
            <div className="lg:flex lg:gap-6 lg:items-start">
              ...
              <div className="mt-4 lg:mt-0 lg:flex-1 text-center py-10 text-[#9CA3AF] text-sm">
                달력에서 날짜를 선택하면<br />해당 날 행사를 볼 수 있어요
              </div>
            </div>
          </div>
        )}
