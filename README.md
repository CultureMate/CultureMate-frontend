...
홈·행사 목록·검색·필터·행사 상세는 기능이 연결되어 있습니다. 코스·관심목록·로그인 화면은 아직 시안입니다. 행사 상세는 원문 링크와 카카오맵을 제공합니다(아래 지도 설정 필요). 목록의 기존 비동작 찜·코스 버튼과 동행인 필터는 이번 검색 범위에서 제외했습니다. 댓글·찜·AI 소개문 생성·조회수 증가 API 연동은 별도 작업입니다. 지도·이미지·글꼴은 외부 서비스를 사용하므로 인터넷 연결 상태에 따라 표시가 달라질 수 있습니다.
